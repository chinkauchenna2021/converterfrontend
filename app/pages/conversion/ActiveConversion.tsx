...
import { fileMimeTypes, fileTypes, IFileOptions } from "@/app/entities/fileTypes";
import {
  UploadedFile,
  useFilesStore,
...
import { IConversion, IDocumentResponse, IFileConversionResponse } from "@/types/Conversion";
...
import { convertBufferToBlob, convertFileToBlob } from "@/app/hooks/convertFileToBlob";
...
function DisplayTabs({
  ...
  section: string;
  onclick: (fileId: string, fromFormat: string, toFormat: string, file:File) => void;
...
function FileFormatPiles({
  ...
  return (
    <div
      onClick={() =>
        onclick(singleFile.id, getFileExtension(singleFile.name), title , singleFile.file)
      }
...
function CustomPopover({
  ...
  children: ReactNode;
  setGetFormat: (fileId: string, fromFormat: string, toFormat: string , file:File) => void;
  singleFile: UploadedFile;
}) {
  return (
    <Popover>
    ...
    <>
      <div className="w-full h-fit py-2 flex justify-between ">
        <div className="flex items-center space-x-6 w-4/6">
          ...
          <Avatar size="sm" isBordered src={URL.createObjectURL(singleFile.file as unknown as File)} />
            {/* <File size={25} className="text-slate-300" /> */}
          </div>
        ...
        {
          isLoading  && currentConversion?
          (
            <CustomButton
            onclick={()=>downloadSingleFile(singleFile.id)}
            styles="capitalize"
            isLoading={ (currentConversion?.fileId == singleFile.id)?isConverting : false}
            title="download"
          />
          )
          :
...
function ActiveConversion() {
  ...
  const conversion = useConversionStore((state) => state.conversions);
  const {mutate, isPending , isError , isSuccess,data:conversionData} = convertWithQuery.convertDocsToHtml()
  const [getResponseData , setResponseData] = useState<IFileConversionResponse[]>([])
 ...
 */
  ...
  const convertAllDocments = async () => {
    ...
    try{
      ...
      const filterConversion =   conversion.map((data)=>({file:data.file,from:data.fromFormat,to:data.toFormat, fileId:data.fileId}))
      console.log(conversion , filterConversion, "conversion collection testing ")
      ...
      */
      ...
      // for(const documents of conversion){
      //    mutate({file:documents.file , from:documents.fromFormat, to:documents.toFormat})
      //    console.log(`from documents` , conversionData)
      //   }
    ...
    // if(isSuccess){
    //   console.log(JSON.stringify(conversionData) , `from the backend server`)
 ...
 */
    ...
    if(isSuccess){
      // console.log(`from documents` , conversionData , mimeType ) 
      console.log(JSON.stringify(conversionData) , `from the backend server`)
  ...
  const downloadSingleFile = async (id: string) => {
    ...
    const mime = conversionData?.responseDocument?.filter((data:IDocumentResponse)=>(data.conversionIndex === id)) as IDocumentResponse[];
    const mimeType = fileMimeTypes[mime[0]?.toFormat] ?? "text/plain"
    try{
      console.log(id,mime, `id from file conversion `)
      const blobData =  convertBufferToBlob(mime[0].fileContent.data as unknown as Buffer, mimeType)       
      // console.log(JSON.stringify(blobData) , "from blob") 
